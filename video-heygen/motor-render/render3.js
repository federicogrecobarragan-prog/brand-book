...
const FPS = 60;
const DURATION = 6.0;
const FRAMES = Math.round(FPS * DURATION); // 360
const W = 1080, H = 1920;

(async () => {
  let html = fs.readFileSync(path.join(DIR, 'intro3-template.html'), 'utf8');
  html = html
  ...
  const finalPath = path.join(DIR, 'intro3-final.html');
  ...
  await page.evaluate(() => window.logoReady); // esperar chroma-key + centrado

  const framesDir = path.join(DIR, 'frames3');
