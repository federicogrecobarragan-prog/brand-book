...
const FRAMES = Math.round(FPS * DURATION); // 90
const W = 1080, H = 1920;

(async () => {
  // inyectar fuentes base64 en el template
  let html = fs.readFileSync(path.join(DIR, 'intro-template.html'), 'utf8');
  html = html
    .replace('__GROTESK700__', fs.readFileSync(path.join(DIR, 'grotesk700.b64'), 'utf8').trim())
    .replace('__GROTESK500__', fs.readFileSync(path.join(DIR, 'grotesk500.b64'), 'utf8').trim())
    .replace('__JETMONO500__', fs.readFileSync(path.join(DIR, 'jetmono500.b64'), 'utf8').trim());
  const finalPath = path.join(DIR, 'intro-final.html');
  ...
  for (let i = 0; i < FRAMES; i++) {
    ...
    if (i % 15 === 0) console.log(`frame ${i}/${FRAMES}`);
  }
